    ...
    this.bot.on('message_callback', async (ctx) => {
      try {
        ...
        if (payload === 'Мой токен' || payload === '🔐 Мой токен') {
          ...
          const user = await require('./authService').findOrCreateByMaxUserId(maxUserId, { name: ctx.user?.full_name || `User ${maxUserId}`, email: `user_${maxUserId}@max.local`, role: 'STUDENT' });
          if (!user) return ctx.reply('Не удалось получить профиль. Попробуйте позже.');
          const token = require('./authService').generateToken(user, '1h');
